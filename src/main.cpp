...
#if defined(NDEBUG)
# error "NTCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc63f2d16fbc40f711322391fd0d0db875f12743d10b7431bcafd256ed35b2e2c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // NTCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "NTCoin Signed Message:\n";
...
{
    // NTCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // NTCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // NTCoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // NTCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // NTCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // NTCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc63f2d16fbc40f711322391fd0d0db875f12743d10b7431bcafd256ed35b2e2c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "14/1/2023 - TechnoVision hits over a million views!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("049889fe231413329e0aea9883737fa1a62e9182756bfa532e9cc284a6fbe6c4d5b249e7474917238936e559dcd2213a7c8f0469d927234f86c8a7ba856c99a6b7") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1673693965;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 385773810;

        if (fTestNet)
        {
            block.nTime    = 1673693866;
            block.nNonce   = 385773810;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x241bfeac37bd3a09c834c72b8661d01382a5778ddea702c4404cac35ac60d699"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb3, 0xda }; // NTCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NTCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NTCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NTCoinMiner(CWallet *pwallet)
{
    printf("NTCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("NTCoin-miner");
    ...
    try { loop {
        ...
        printf("Running NTCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NTCoinMiner, pwallet));
}
